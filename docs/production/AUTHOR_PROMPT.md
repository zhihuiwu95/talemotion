...
版本选择：读取 capabilities.json 的 catalogs[任务指定版本]；latest 仅推荐，不做自动迁移。本项目当前锁步支持 1.0/1.0 与 1.1/1.1；1.0 不得使用新增背景或 soundCue。1.1 的雨后背景、40 节点、beat 起点、音效 ID 是本需求 E2 契约，背景已完成 E3 本地实现、混音管线 E4 尚未实现，不能因目录存在就交付可播放新故事。E2 确认后变更字段/能力必须回到 E2，普通作者不能自行扩展。详细范围见 STORY_STANDARD 的版本表。
