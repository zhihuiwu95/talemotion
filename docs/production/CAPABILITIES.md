...
- 背景：`snow` 雪地、`rain` **正在下雨**的草地（可见雨线与循环动画）、`meadow` 晴天草地。雨后故事可用 meadow 加口头说明；没有单独的雨后湿地资产。
...
制作提示：`line.text` 才会生成配音；`cue` 和 choice 的 `label` 不自动朗读。选择目标只有实体，当前没有可点的“空地”区域；借物体代表地点时要口头说明对应关系。
