...
机器目录现为 formatVersion 2.0：按 `catalogs[story.catalogVersion]` 读取完整清单，latest 仅为制作推荐。1.0/1.0 与 1.1/1.1 显式分支，旧包不改写。1.1 新能力现处于 E2 契约阶段，待配套实现后才能发布；详见 [能力目录说明](CAPABILITIES.md) 的版本表。旧扁平目录消费者需迁移，外部工具未核验。
